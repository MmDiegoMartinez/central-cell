...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo_excel'])) {
    ...
    if ($archivo['error'] !== UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($archivo['tmp_name'], $rutaTemporal)) {
            $resultado    = procesarArchivoExceltel($rutaTemporal);
            @unlink($rutaTemporal);
...
<div class="page">
  ...
  <div class="header">
    ...
    <div>
      <h1>Cargar Existencias Telefonos</h1>
...
form.addEventListener('submit', () => {
  ...
  function advance() {
    ...
    // El último step se mantiene hasta que el server responda
